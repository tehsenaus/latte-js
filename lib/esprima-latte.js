(function (root, factory) {
    ...
    SyntaxTreeDelegate = {
        ...
        createForInStatement: function (left, right, body, test) {
            return {
        ...
        createForOfStatement: function (left, right, body, test) {
            return {
    ...
    function parseArrayInitialiser() {
        ...
        while (!match(']')) {
            if (lookahead.value === 'for' && lookahead.type === Token.Keyword) {
                ...
                expect(')');
            } else if (lookahead.value === 'with' && lookahead.type === Token.Keyword) {
                if (!possiblecomprehension) {
                    throwError({}, Messages.ComprehensionError);
                }
                expectKeyword('with');
                tmp = parseAssignmentExpression();
                tmp.type = 'ComprehensionBlockVar';
                blocks.push(tmp);
                ...
                } else if (!(match(']') || matchKeyword('for') || matchKeyword('if') || matchKeyword('with'))) {
                    expect(',');
                    possiblecomprehension = false;
                }
            }
    ...
    function parseMonadExpression() {
        var body, test, oldInMonad;
        expectKeyword('do');
        oldInMonad = state.inMonad;
        state.inMonad = true;
        body = parseStatement();
        state.inMonad = oldInMonad;
        return {
    ...
    function parseForStatement(opts) {
        ...
        if (typeof left !== 'undefined' && matchKeyword('if')) {
            lex();
            test = parseExpression();
        }
        expect(')');
        ...
        if (operator.value === 'in') {
            return delegate.createForInStatement(left, right, body, test);
        }
        return delegate.createForOfStatement(left, right, body, test);
    }
    ...
    function parseReturnStatement() {
        ...
        if (!state.inFunctionBody && !state.inMonad) {
