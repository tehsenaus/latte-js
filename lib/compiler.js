...
compiler.files = function (options) {
    ...
    return es.pipeline(es.split(), stream = es.map(function (file, cb) {
        ...
        mkdirp(path.dirname(outputPath), null, function (err) {
            ...
            compiler.pipe(fs.createWriteStream(outputPath).on('error', function (e) {
                cb(file + ': ' + e);
            }));
        });
    }), es.join('\n'));
...
function compileFile(file, options) {
    ...
    return fs.createReadStream(file).on('error', compilerStream.emit.bind(compilerStream, 'error')).pipe(compilerStream);
}
