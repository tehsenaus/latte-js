var traverser = require('../traverser'), esprima = require('../esprima-latte'), fs = require('fs'), path = require('path');
var preludes = {};
fs.readdirSync(path.join(__dirname, '../prelude')).forEach(function (file) {
    var src = fs.readFileSync(path.join(__dirname, '../prelude', file), 'utf8'), ast = esprima.parse(src);
