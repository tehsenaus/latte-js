        ...
        forInStmt: function (stmt, context) {
            if (stmt.left.type == 'VariableDeclaration' && stmt.left.kind == 'let') {
                ...
                if (stmtHasBreakContinue(body) || stmtHasReturn(body)) {
                    var t = gen.ident(context.temp()), id = stmt.left.declarations[0].id;
                    stmt.left.kind = 'var';
                    stmt.left.declarations[0].id = t;
                    body.body.unshift(gen.varDecl(id, t, { kind: 'let' }));
                    stmt.body = body;
                } else {
                    stmt = gen.forEachKey(stmt.right, body.body, stmt.left.declarations[0].id);
                }
